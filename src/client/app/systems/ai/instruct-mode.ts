...
import { parseExampleIntoIndividual } from "/app/systems/ai/openai";
import { power_user, context_presets } from "/app/systems/power-user";
import { onlyUnique, regexFromString, resetScrollHeight } from "/app/systems/shared/utils";
...
export function formatInstructModeExamples(mesExamplesArray: any, name1: any, name2: any) {
  ...
  const includeGroupNames =
    selected_group &&
    [names_behavior_types.ALWAYS, names_behavior_types.FORCE].includes(power_user.instruct.names_behavior);
  ...
  for (const item of mesExamplesArray) {
    ...
    for (const example of blockExamples) {
      ...
      const includeThisName =
        !includeGroupNames &&
        (includeNames ||
          (power_user.instruct.names_behavior === names_behavior_types.FORCE && example.name == "example_user"));
 ...
 * @returns {import('/app/systems/macros').Macro[]} Macro objects.
 */
...
jQuery(() => {
  ...
  $("#instruct_derived").on("change", () => {
    $("#instruct_derived").parent().find("i").toggleClass("toggleEnabled", !!power_user.instruct_derived);
  });

  $("#instruct_bind_to_context").on("change", () => {
