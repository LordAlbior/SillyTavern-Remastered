...
import { isTrueBoolean } from "/app/systems/shared/utils";
 ...
 * @param {import('/app/systems/slash-commands/slash-commands/SlashCommandScope').SlashCommandScope} scope The scope to assign variables to.
...
export class ToolManager {
   ...
   */
  static get tools() {
    return Array.from(ToolManager.#tools.values());
  }
  ...
  static registerFunctionTool({
    ...
    ToolManager.#tools.set(name, definition);
  ...
  static async invokeFunctionTool(name: any, parameters: any) {
    try {
      ...
      const invokeParameters = ToolManager.#parseParameters(parameters);
      const tool = ToolManager.#tools.get(name);
  ...
  static async formatToolCallMessage(name: any, parameters: any) {
    ...
    try {
      const tool = ToolManager.#tools.get(name);
      const formatParameters = ToolManager.#parseParameters(parameters);
  ...
  static parseToolCalls(toolCalls: any, parsed: any, toolSignatures: any = {}) {
    ...
    if (typeof parsed?.delta === "object") {
      ...
      if (targetToolCall) {
        if (parsed?.delta?.type === "input_json_delta") {
          ...
          if (!targetToolCall[ToolManager.#INPUT_DELTA_KEY]) {
            targetToolCall[ToolManager.#INPUT_DELTA_KEY] = "";
          }
          targetToolCall[ToolManager.#INPUT_DELTA_KEY] += jsonDelta;
        }
      }
    }
    if (parsed?.type === "content_block_stop") {
      ...
      const targetToolCall = toolCalls[choiceIndex]?.[toolCallIndex];
      if (targetToolCall) {
        const jsonDeltaString = targetToolCall[ToolManager.#INPUT_DELTA_KEY];
        if (jsonDeltaString) {
          try {
            const jsonDelta = { input: JSON.parse(jsonDeltaString) };
            delete targetToolCall[ToolManager.#INPUT_DELTA_KEY];
