...
import { autoSelectInstructPreset } from "/app/systems/ai/instruct-mode";

import { power_user } from "/app/systems/power-user";
import { getEventSourceStream } from "/app/systems/shared/sse-stream";
import { getSortableDelay, versionCompare } from "/app/systems/shared/utils";
...
export function loadKoboldSettings(data: any, preset: any, settings: any) {
  ...
  if (kai_settings.preset_settings == "gui") {
    ...
    if (typeof koboldai_setting_names[kai_settings.preset_settings] !== "undefined") {
      $(`#settings_preset option[value=${koboldai_setting_names[kai_settings.preset_settings]}]`).attr(
        "selected",
        "true",
      );
    } else {
...
export function getKoboldGenerationData(finalPrompt: any, settings: any, maxLength: any, maxContextLength: any, isHorde: any, type: any) {
  ...
  const generate_data = {
    ...
    singleline: false,
    stop_sequence:
      kai_flags.can_use_stop_sequence || isHorde ? getStoppingStrings(isImpersonate, isContinue) : undefined,
    ...
    use_default_badwordsids:
      kai_flags.can_use_default_badwordsids || isHorde ? kai_settings.use_default_badwordsids : undefined,
