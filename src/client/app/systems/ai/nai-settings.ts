...
import { MAX_CONTEXT_DEFAULT, MAX_RESPONSE_DEFAULT, power_user } from "/app/systems/power-user";
import { getTextTokens, tokenizers } from "/app/systems/ai/tokenizers";
import { getEventSourceStream } from "/app/systems/shared/sse-stream";
import { getSortableDelay, getStringHash, onlyUnique } from "/app/systems/shared/utils";
...
function loadNovelSettingsUi(ui_settings: any) {
  ...
  $(`#settings_preset_novel option[value=${novelai_setting_names[nai_settings.preset_settings_novel]}]`).prop(
    "selected",
    true,
  );
...
export function getNovelGenerationData(finalPrompt: any, settings: any, maxLength: any, isImpersonate: any, isContinue: any, _cfgValues: any, type: any) {
  ...
  const badWordIds =
    tokenizerType !== tokenizers.NONE ? getBadWordIds(nai_settings.banned_tokens, tokenizerType) : undefined;
...
$("#nai_preamble_textarea").on("input", () => {
  nai_settings.preamble = String($("#nai_preamble_textarea").val());
  saveSettingsDebounced();
});

$("#nai_preamble_restore").on("click", () => {
...
export function initNovelAISettings() {
  ...
  $("#api_button_novel").on("click", async (e) => {
    e.stopPropagation();
  ...
  $("#settings_preset_novel").on("change", async () => {
