...
import { getCurrentLocale, t, translate } from "/app/systems/shared/i18n";
import { macros, MacroCategory } from "/app/systems/macros/macro-system";
import { chat_completion_sources, getChatCompletionModel, oai_settings } from "/app/systems/ai/openai";
import { Popup } from "/app/systems/ui/popup";
import { performFuzzySearch, power_user } from "/app/systems/power-user";
    ...
    case "openai":
      ...
      switch (chatCompletionSource ?? oai_settings.chat_completion_source) {
        ...
        case chat_completion_sources.OPENROUTER:
          return data?.choices?.[0]?.message?.reasoning ?? data?.choices?.[0]?.message?.reasoning_content ?? "";
...
export class ReasoningHandler {
  ...
  updateReasoning(messageId: any, reasoning: any = null, { persist = false, allowReset = false } = {}) {
    ...
    this.type =
      this.#isParsingReasoning || this.#parsingReasoningMesStartIndex ? ReasoningType.Parsed : ReasoningType.Model;
    ...
    SlashCommand.fromProps({
      ...
      unnamedArgumentList: [
        SlashCommandArgument.fromProps({
          ...
          typeList: [ARGUMENT_TYPE.STRING],
          enumProvider: () =>
            reasoning_templates.map((x) => new SlashCommandEnumValue(x.name, null, enumTypes.enum, enumIcons.preset)),
        }),
      ],
      helpString: `
            <div>
...
function setReasoningEventHandlers() {
  ...
  $(document).on("click", ".mes_reasoning_details", (e) => {
  ...
  $(document).on("click", ".mes_reasoning_close_all", (e) => {
    e.stopPropagation();
    e.preventDefault();
