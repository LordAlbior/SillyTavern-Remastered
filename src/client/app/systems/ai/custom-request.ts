...
import { extractReasoningFromData } from "/app/systems/ai/reasoning";
import { formatInstructModeChat, formatInstructModePrompt, getInstructStoppingSequences } from "/app/systems/ai/instruct-mode";
import {
  getStreamingReply,
...
import EventSourceStream from "/app/systems/shared/sse-stream";
...
export class TextCompletionService {
  ...
  static async processRequest(requestData: any, options = {}, extractData = true, signal = null) {
    ...
    requestData = TextCompletionService.createRequestData(requestData);
    ...
    if (Array.isArray(prompt)) {
      if (instructName) {
        ...
        if (instructPreset) {
          requestData.prompt = TextCompletionService.constructPrompt(
            prompt,
            instructPreset,
            (options as any).instructSettings,
          );
    ...
    if (presetName) {
      const presetManager = getPresetManager(TextCompletionService.TYPE);
      if (presetManager) {
        const preset = presetManager.getCompletionPresetByName(presetName);
        if (preset) {
          // Convert preset to payload and merge with custom data
          requestData = TextCompletionService.presetToGeneratePayload(preset, {}, requestData);
        } else {
    ...
    const response = await TextCompletionService.sendRequest(requestData, extractData, signal);
    ...
    if (!requestData.stream && extractData) {
      ...
      if (instructPreset) {
        ...
        [instructPreset.output_sequence, instructPreset.last_output_sequence].forEach((sequences) => {
          if (sequences) {
            sequences
              .split("\n")
  ...
  static presetToGeneratePayload(preset: any, overridePreset: any = {}, overridePayload: any = {}) {
    ...
    const payload = createTextGenGenerationData(
      settings,
      (overridePayload as any).model,
      (overridePayload as any).prompt,
      preset.genamt,
    );

    // apply overrides
    return TextCompletionService.createRequestData({ ...payload, ...overridePayload } as any);
  }
}
...
export class ChatCompletionService {
  ...
  static async processRequest(requestData: any, options: any, extractData = true, signal = null) {
    ...
    // Apply generation preset if specified
    if (presetName) {
      const presetManager = getPresetManager(ChatCompletionService.TYPE);
      if (presetManager) {
        const preset = presetManager.getCompletionPresetByName(presetName);
        if (preset) {
          // Convert preset to payload and merge with custom parameters
          requestData = await ChatCompletionService.presetToGeneratePayload(preset, {}, requestData);
        } else {
          console.warn(`Preset "${presetName}" not found, continuing with default settings`);
        }
      } else {
    ...
    return await ChatCompletionService.sendRequest(requestData, extractData, signal);
  }
  ...
  static async presetToGeneratePayload(preset: any, overridePreset: any = {}, overridePayload: any = {}) {
    ...
    const data = await createGenerationParameters(
      ...
      (overridePayload as any).model,
      "quiet",
      (overridePayload as any).messages,
    );
    const payload = data.generate_data;

    // apply overrides
    return ChatCompletionService.createRequestData({ ...payload, ...overridePayload } as any);
  }
}
