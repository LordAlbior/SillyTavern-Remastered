...
//import { BIAS_CACHE, displayLogitBias, getLogitBiasListResult } from '/app/systems/ai/logit-bias';
//import { getEventSourceStream } from '/app/systems/shared/sse-stream';
//import { getSortableDelay, onlyUnique } from '/app/systems/shared/utils';
//import { getCfgPrompt } from '/app/systems/shared/cfg-scale';
import {
...
export function getActiveManualApiSamplers(tcApiType = "") {
  ...
  try {
    ...
    return presetSamplers.filter(([key, val]) => val === true && key !== "st_manual_priority").map(([key, val]) => key);
  } catch (error) {
