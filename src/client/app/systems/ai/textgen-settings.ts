...
import { deriveTemplatesFromChatTemplate } from "/app/systems/prompts/chat-templates";
import { t } from "/app/systems/shared/i18n";
import { autoSelectInstructPreset, selectContextPreset, selectInstructPreset } from "/app/systems/ai/instruct-mode";
import { BIAS_CACHE, createNewLogitBiasEntry, displayLogitBias, getLogitBiasListResult } from "/app/systems/ai/logit-bias";
...
} from "/app/systems/ai/samplerSelect";
import { SECRET_KEYS, writeSecret } from "/app/systems/shared/secrets";
...
import { getSortableDelay, onlyUnique, arraysEqual, isObject } from "/app/systems/shared/utils";
...
function getCustomTokenBans(settings: any = null) {
  ...
  if (
    ...
    (!settings.banned_tokens && !settings.global_banned_tokens && !textgenerationwebui_banned_in_macros.length)
  ) {
    return {
      banned_tokens: "",
      banned_strings: [],
...
export function initTextGenSettings() {
  ...
  $("#koboldcpp_default_order").on("click", () => {
  ...
  $("#llamacpp_samplers_default_order").on("click", () => {
  ...
  $("#textgenerationwebui_default_order").on("click", () => {
  ...
  $("#aphrodite_default_order").on("click", () => {
      ...
      for (const [id, value] of Object.entries(inputs)) {
        ...
        if (inputElement.prop("type") === "checkbox") {
          ...
          if (power_user.enableZenSliders) {
            ...
            const zenSlider = ($(`#${masterElementID}_zenslider`) as any).slider();
  ...
  $("#openrouter_allow_fallbacks_textgenerationwebui").on("input", () => {
    updateOpenRouterProvidersWarning("#openrouter_providers_text");
  });
  ...
  $("#api_button_textgenerationwebui").on("click", async (e) => {
    const keys = [
...
function setSettingByName(setting: any, value: any, trigger: any) {
  ...
  const isText =
    $(`#${setting}_textgenerationwebui`).attr("type") == "text" || $(`#${setting}_textgenerationwebui`).is("textarea");
  if (isCheckbox) {
    ...
    if (power_user.enableZenSliders) {
      const zenSlider = ($(`#${setting}_textgenerationwebui_zenslider`) as any).slider();
...
export function createTextGenGenerationData(
  ...
  const aphroditeParams = {
    ...
    sampler_priority:
      settings.type === APHRODITE && !arraysEqual(settings.samplers_priorities, APHRODITE_DEFAULT_ORDER)
  ...
  if (settings.type === HUGGINGFACE) {
    ...
    nonAphroditeParams.seed = settings.seed >= 0 ? settings.seed : Math.floor(Math.random() * 2 ** 32);
  }
  ...
  if (settings.type === LLAMACPP || settings.type === OLLAMA) {
    ...
    const logitBiasArray =
      params.logit_bias && typeof params.logit_bias === "object" && Object.keys(params.logit_bias).length > 0
