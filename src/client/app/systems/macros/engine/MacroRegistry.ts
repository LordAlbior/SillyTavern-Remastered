...
class MacroRegistry {
  ...
  executeMacro(call: any, { defOverride }: any = {}) {
    ...
    if (!isArgsValid(def, args)) {
      ...
      const expectedMax =
        def.list && def.list.max !== null ? def.maxArgs + def.list.max : def.list ? null : def.maxArgs;
  ...
  buildMacroDefFromOptions(name: any, options: any, { source }: any = {}) {
    ...
    if (rawAliases !== undefined && rawAliases !== null) {
      ...
      for (const [i, aliasDef] of rawAliases.entries()) {
        ...
        if (typeof aliasDef.alias !== "string" || !aliasDef.alias.trim())
          throw new Error(`Macro "${name}" options.aliases[${i}].alias must be a non-empty string.`);
    ...
    if (rawList !== undefined) {
      if (typeof rawList === "boolean") {
        ...
        if (typeof rawList.min !== "number" || rawList.min < 0)
          throw new Error(`Macro "${name}" options.list.min must be a non-negative integer when provided.`);
        ...
        if (rawList.max !== undefined && rawList.max < rawList.min)
          throw new Error(`Macro "${name}" options.list.max must be greater than or equal to options.list.min.`);
    ...
    if (rawDelayArgResolution !== undefined) {
      if (typeof rawDelayArgResolution !== "boolean")
        throw new Error(`Macro "${name}" options.delayArgResolution must be a boolean when provided.`);
      delayArgResolution = rawDelayArgResolution;
    }
...
function detectMacroSource() {
  ...
  if (isThirdParty) {
    ...
    const callerIdx = stack.findIndex((line) => line.includes("registerMacro") && line.includes("MacroRegistry"));
