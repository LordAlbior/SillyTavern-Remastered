...
import { favsToHotswap } from "/app/systems/shared/RossAscends-mods";
import { loader } from "/app/systems/shared/action-loader";
import { convertCharacterToPersona } from "/app/systems/characters/personas";
...
class CharacterContextMenu {
  ...
  static favorite = async (characterId: any) => {
    ...
    if (!mergeResponse.ok) {
      mergeResponse
        .json()
        .then((json) => toastr.error(`Character not saved. Error: ${json.message}. Field: ${json.error}`));
    }
...
class BulkEditOverlay {
  ...
  #stopEventPropagation = (event: any) => {
    ...
    this.#getDisabledElements().forEach((element) => element.removeEventListener("click", this.#stopEventPropagation));

  #disableClickEventsForGroups = () =>
    this.#getDisabledElements().forEach((element) => element.addEventListener("click", this.#stopEventPropagation));

  #enableClickEventsForCharacters = () =>
    this.#getEnabledElements().forEach((element) => element.removeEventListener("click", this.toggleCharacterSelected));
  ...
  handleContextMenuDuplicate = () =>
    Promise.all(this.selectedCharacters.map(async (characterId) => CharacterContextMenu.duplicate(characterId)))
      .then(() => getCharacters())
