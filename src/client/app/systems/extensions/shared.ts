...
import { extension_settings, openThirdPartyExtensionMenu } from "./index";
import { t } from "/app/systems/shared/i18n";
import { oai_settings, proxies, ZAI_ENDPOINT } from "/app/systems/ai/openai";
import { SECRET_KEYS, secret_state } from "/app/systems/shared/secrets";
import { textgen_types, textgenerationwebui_settings } from "/app/systems/ai/textgen-settings";
import { getTokenCountAsync } from "/app/systems/ai/tokenizers";
import { createThumbnail, isValidUrl } from "/app/systems/shared/utils";
...
export class ConnectionManagerRequestService {
   ...
   * @param {string | (import('/app/systems/ai/custom-request').ChatCompletionMessage & {ignoreInstruct?: boolean})[]} prompt
   ...
   * @returns {Promise<import('/app/systems/ai/custom-request').ExtractedData | (() => AsyncGenerator<import('/app/systems/ai/custom-request').StreamResponse>)>} If not streaming, returns extracted data; if streaming, returns a function that creates an AsyncGenerator
   */
  static async sendRequest(
    ...
    const { stream, signal, extractData, includePreset, includeInstruct, instructSettings } = {
      ...ConnectionManagerRequestService.defaultSendRequestParams,
      ...custom,
    };
  ...
  static getProfileIcon(profileId: any) {
    ...
    try {
      const profile = ConnectionManagerRequestService.getProfile(id);
  ...
  static isProfileSupported(profile: any) {
    ...
    if (!Object.hasOwn(ConnectionManagerRequestService.getAllowedTypes(), apiMap.selected)) {
      return false;
    }
  ...
  static validateProfile(profile: any) {
    ...
    if (!Object.hasOwn(ConnectionManagerRequestService.getAllowedTypes(), selectedApiMap.selected)) {
      throw new Error(
        `API type ${selectedApiMap.selected} is not supported. Supported types: ${Object.values(ConnectionManagerRequestService.getAllowedTypes()).join(", ")}`,
      );
    }
  ...
  static handleDropdown(
    ...
    for (const [apiType, groupLabel] of Object.entries(ConnectionManagerRequestService.getAllowedTypes())) {
    ...
    for (const apiType of Object.keys(ConnectionManagerRequestService.getAllowedTypes())) {
    ...
    context.eventSource.on(context.eventTypes.CONNECTION_PROFILE_UPDATED, async (oldProfile: any, newProfile: any) => {
      ...
      if (!ConnectionManagerRequestService.isProfileSupported(newProfile)) {
