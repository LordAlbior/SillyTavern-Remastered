...
import { isTrueBoolean, resetScrollHeight } from "/app/systems/shared/utils";
...
async function migrateSystemPromptFromInstructMode() {
  if ("system_prompt" in power_user.instruct) {
    ...
    toastr.info("System prompt settings have been moved from the Instruct Mode.", "Migration notice", {
      timeOut: 5000,
    });
  }
}
    ...
    SlashCommand.fromProps({
      ...
      unnamedArgumentList: [
        SlashCommandArgument.fromProps({
          ...
          enumProvider: () =>
            system_prompts.map((x) => new SlashCommandEnumValue(x.name, null, enumTypes.enum, enumIcons.preset)),
        }),
      ],
      helpString: `
            <div>
