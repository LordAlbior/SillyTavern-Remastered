...
import { getLastMessageId } from "/app/systems/shared/macros";
...
import { createTagMapFromList } from "/app/systems/characters/tags";
...
import { getUniqueName, isTrueBoolean } from "/app/systems/shared/utils";
...
export async function createBranch(mesId: any, { swipeId = null } = {}) {
  ...
  if (
    ...
    (!Number.isInteger(selectedSwipeId) || selectedSwipeId < 0 || selectedSwipeId >= (lastMes?.swipes?.length ?? 0))
  ) {
...
export async function convertSoloToGroupChat() {
  ...
  const confirm = await Popup.show.confirm(
    ...
    t`Are you sure you want to convert this chat to a group chat?` + "<br />" + t`This cannot be reverted.`,
  );
  if (!confirm) {
    return;
  }
