...
import { timestampToMoment, isDigitsOnly, getStringHash, escapeRegex, uuidv4 } from "/app/systems/shared/utils";
import { textgenerationwebui_banned_in_macros } from "/app/systems/ai/textgen-settings";
import { getInstructMacros } from "/app/systems/ai/instruct-mode";
import { getVariableMacros } from "/app/systems/characters/variables";
import { isMobile } from "/app/systems/shared/RossAscends-mods";
import { inject_ids } from "/app/systems/shared/constants";
import { initRegisterMacros, macros as macroSystem } from "/app/systems/macros/macro-system";
...
export class MacrosParser {
  ...
  static registerMacro(key: any, value: any, description: any = "") {
    ...
    if (MacrosParser.#macros.has(key)) {
      console.warn(`Macro ${key} is already registered`);
    }
    ...
    if (typeof description === "string" && description) {
      MacrosParser.#descriptions.set(key, description);
    }
  }
  ...
  static unregisterMacro(key: any) {
    ...
    const deleted = MacrosParser.#macros.delete(key);
...
function getPickReplaceMacro(rawContent: any) {
  ...
  const pickReplace = (match: any, listString: any, offset: any) => {
    ...
    // @ts-expect-error - have to use numbers for legacy picks
