...
import { extension_settings, writeExtensionField } from "/app/systems/extensions";
import { getPresetManager } from "/app/systems/shared/preset-manager";
import { regexFromString } from "/app/systems/shared/utils";
...
/**
 * @typedef {import('/app/systems/characters/char-data').RegexScriptData} RegexScript
 */
...
export function getScriptsByType(
  ...
  switch (scriptType) {
    ...
    case SCRIPT_TYPES.PRESET: {
      if (
        allowedOnly &&
        !extension_settings?.preset_allowed_regex?.[getCurrentPresetAPI()]?.includes(getCurrentPresetName())
      ) {
        return [];
      }
