...
import { renderExtensionTemplateAsync } from "/app/systems/extensions";
import { POPUP_RESULT, POPUP_TYPE, callGenericPopup } from "/app/systems/ui/popup";
...
import { isValidUrl } from "/app/systems/shared/utils";
  ...
  async scrape(): Promise<File[] | undefined> {
    ...
    if (output === "multi") {
      ...
      for (const attachment of data) {
        const file = new File([String(attachment.content).trim()], `${String(attachment.title).trim()}.txt`, {
          type: "text/plain",
        });
        files.push(file);
      }
