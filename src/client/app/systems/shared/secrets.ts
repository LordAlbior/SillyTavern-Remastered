...
import { t, registerSecretDisplayHook } from "/app/systems/shared/i18n";
import { chat_completion_sources } from "/app/systems/ai/openai";
...
import { textgen_types } from "/app/systems/ai/textgen-settings";
import { getCurrentUserHandle } from "/app/systems/shared/user";
import { copyText, isTrueBoolean, uuidv4 } from "/app/systems/shared/utils";
...
// Inverted dependency: secrets provides its display-refresh to i18n (breaks i18n ↔ secrets cycle).
registerSecretDisplayHook(updateSecretDisplay);
...
/**
 * @type {import('/app/systems/shared/secrets').SecretStateMap}
 */
...
async function openKeyManagerDialog(key: any) {
  ...
  async function renderSecretsList() {
    ...
    for (const secret of secrets) {
      ...
      itemTemplate.find('[data-action="copy-id"]').on("click", async () => {
        await copyText(secret.id);
        toastr.info(t`Secret ID copied to clipboard.`);
      });
      itemTemplate.find('button[data-action="rotate-secret"]').on("click", async () => {
        await rotateSecret(key, secret.id);
        await renderSecretsList();
      });
      itemTemplate.find('button[data-action="copy-secret"]').on("click", async () => {
      ...
      itemTemplate.find('button[data-action="rename-secret"]').on("click", async () => {
      ...
      itemTemplate.find('button[data-action="delete-secret"]').on("click", async () => {
