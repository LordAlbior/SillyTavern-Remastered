...
import { extension_settings, saveMetadataDebounced } from "/app/systems/extensions";
import { selected_group } from "/app/systems/chat/group-chats";
import { getCharaFilename, delay } from "/app/systems/shared/utils";
...
function setCharCfg(tempValue: any, setting: any) {
  ...
  // Assign temp object
  const tempCharaCfg: Record<string, any> = {
    name: avatarName,
  };
...
export function initCfg() {
  ...
  $('input[name="cfg_prompt_combine"]').on("input", () => {
    const values =
      $("#cfgConfig")
...
export function getCfgPrompt(guidanceScale: any, isNegative: any, quiet = false) {
  ...
  if (guidanceScale.type === cfgType.chat || cfgPromptCombine.includes(cfgType.chat)) {
    splitCfgPrompt.unshift(
      substituteParams(chat_metadata[isNegative ? metadataKeys.negative_prompt : metadataKeys.positive_prompt]),
    );
  }
