...
import { instruct_presets } from "/app/systems/ai/instruct-mode";
import { kai_settings } from "/app/systems/ai/kai-settings";
import { convertNovelPreset } from "/app/systems/ai/nai-settings";
import { oai_settings, openai_setting_names, openai_settings } from "/app/systems/ai/openai";
import { POPUP_RESULT, POPUP_TYPE, Popup } from "/app/systems/ui/popup";
import { context_presets, getContextSettings, power_user } from "/app/systems/power-user";
import { reasoning_templates } from "/app/systems/ai/reasoning";
...
import { checkForSystemPromptInInstructTemplate, system_prompts } from "/app/systems/prompts/sysprompt";
...
class PresetManager {
  ...
  static async performMasterImport(data: any, fileName: any) {
    ...
    // 1. Instruct Template
    if (PresetManager.isPossiblyInstructData(data)) {
    ...
    // 2. Context Template
    if (PresetManager.isPossiblyContextData(data)) {
    ...
    // 3. System Prompt
    if (PresetManager.isPossiblySystemPromptData(data)) {
    ...
    if (PresetManager.isPossiblyTextCompletionData(data)) {
    ...
    if (PresetManager.isPossiblyReasoningData(data)) {
    ...
    const validSections = [];
    for (const [key, section] of Object.entries(PresetManager.masterSections)) {
...
export async function initPresetManager() {
  ...
  $("#af_master_import_file").on("change", async (e) => {
