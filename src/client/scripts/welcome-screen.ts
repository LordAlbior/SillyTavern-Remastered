...
class PinnedChatsManager {
  ...
  static toggle(recentChat: any, pinned: any) {
    ...
    PinnedChatsManager.#saveState(pinState);
  }
  ...
  static rename(recentChat: any, newFileName: any) {
    ...
    const newKey = PinnedChatsManager.getKey(updatedChat);
    pinState[newKey] = {
      group: recentChat.group,
      avatar: recentChat.avatar,
  ...
  await callGenericPopup(t`Recent Chats Settings`, POPUP_TYPE.CONFIRM, null as any, {
    ...
    onClose: (popup: any) => {
      ...
      const collapsedInputValue =
        popup.inputResults.get(collapsedRecentChatsInput.id)?.toString() ?? String(DEFAULT_COLLAPSED_DISPLAYED);
