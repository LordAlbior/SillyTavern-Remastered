...
import { getFriendlyTokenizerName, getTokenCountAsync } from "./tokenizers";
...
export async function itemizedParams(itemizedPrompts: any, thisPromptSet: any, incomingMesId: any) {
  ...
  if (params.this_main_api == "openai") {
    ...
    params.storyStringTokensPercentage = (
      ((params.afterScenarioAnchorTokens + params.beforeScenarioAnchorTokens + params.oaiPromptTokens) /
        params.finalPromptTokens) *
      100
    ).toFixed(2);
    ...
    params.worldInfoStringTokensPercentage = ((params.worldInfoStringTokens / params.finalPromptTokens) * 100).toFixed(
      2,
    );
    ...
    params.storyStringTokens =
      (await getTokenCountAsync(itemizedPrompts[thisPromptSet].storyString)) - params.worldInfoStringTokens;
    ...
    params.ActualChatHistoryTokens =
      ...
      (params.allAnchorsTokens - (params.beforeScenarioAnchorTokens + params.afterScenarioAnchorTokens)) +
    ...
    params.selectedTokenizer =
      itemizedPrompts[thisPromptSet]?.tokenizer || getFriendlyTokenizerName(params.this_main_api).tokenizerName;
  }
  return params;
}
...
export async function promptItemize(itemizedPrompts: any, requestedMesId: any) {
  ...
  if (priorPromptArrayItemForRawPromptDisplay >= 0) {
    ...
    diffPrevPrompt.addEventListener("click", () => {
  ...
  popup.dlg.querySelector("#copyPromptToClipboard").addEventListener("pointerup", async () => {
  ...
  popup.dlg.querySelector("#showRawPrompt").addEventListener("click", async () => {
