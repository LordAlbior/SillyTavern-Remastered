...
export class AutoComplete {
  ...
  basicAutoHideCheck() {
    ...
    return (
      ...
      this.parserResult.start + this.parserResult.name.length + (this.startQuote ? 1 : 0) + (this.endQuote ? 1 : 0) + 1
    );
  }
  ...
  async handleKeyDown(evt: any) {
    ...
    if (this.isActive && this.isReplaceable) {
      ...
      switch (evt.key) {
        ...
        case "Tab": {
          // pick the selected item to autocomplete
          if ((power_user.stscript.autocomplete.select & AUTOCOMPLETE_SELECT_KEY.TAB) != AUTOCOMPLETE_SELECT_KEY.TAB)
            break;
          if (evt.ctrlKey || evt.altKey || evt.shiftKey || this.selectedItem.value == "") break;
          evt.preventDefault();
          evt.stopImmediatePropagation();
