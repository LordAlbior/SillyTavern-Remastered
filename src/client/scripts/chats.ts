...
import { DragAndDropHandler } from "./dragdrop";
...
import { MEDIA_DISPLAY, MEDIA_SOURCE, MEDIA_TYPE, SCROLL_BEHAVIOR, SWIPE_DIRECTION } from "./constants";
...
async function moveAttachment(attachment: any, source: any, callback: any) {
  ...
  const template = $(
    await renderExtensionTemplateAsync("attachments", "move-attachment", { name: attachment.name, targets }),
  );
  template
...
async function openAttachmentManager() {
  ...
  const dragDropHandler = new DragAndDropHandler(".popup", async (files: any, event: any) => {
    ...
    const targetSelectTemplate = $(
      await renderExtensionTemplateAsync("attachments", "files-dropped", { count: files.length, targets: targets }),
    );
...
export function initChatUtilities() {
  ...
  $(document).on("click", ".assistant_note_export", async (_e) => {
    /** @type {ChatHeader} */
    const chatHeader = {
    ...
    const chatToSave = [chatHeader, ...chat.filter((x) => x?.extra?.type !== system_message_types.ASSISTANT_NOTE)];
  ...
  $(document).on("click", "#manageAttachments", () => {
  ...
  $(document).on("input", "#forbid_media_override_allowed", () => {
    const entityId = getCurrentEntityId();
    if (!entityId) return;
  ...
  $(document).on("input", "#forbid_media_override_forbidden", () => {
    const entityId = getCurrentEntityId();
    if (!entityId) return;
  ...
  $(document).on("input", "#forbid_media_override_global", () => {
    const entityId = getCurrentEntityId();
    if (!entityId) return;
    power_user.external_media_allowed_overrides = power_user.external_media_allowed_overrides.filter(
      (v: any) => v !== entityId,
  ...
  $("#creators_note_styles_button").on("click", (e) => {
    e.stopPropagation();
