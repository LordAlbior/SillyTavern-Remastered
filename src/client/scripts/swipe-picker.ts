import { branchChat } from "./bookmarks";
import { SWIPE_DIRECTION, SWIPE_SOURCE } from "./constants";
import { t } from "./i18n";
...
import { addLongPressEvent, clamp, copyText, timestampToMoment } from "./utils";
import {
  chat,
  deleteSwipe,
  ensureSwipes,
...
async function openSwipePicker(messageId: any) {
    ...
    const swipeBlocks = await Promise.all(
      message.swipes.map(async (swipe: any, index: any) => {
        ...
        template
          ...
          .text(`#${index + 1}${index === Number(message.swipe_id ?? 0) ? ` ${t`[Current]`}` : ""}`);
        template.find(".chat_messages_date").text(sendDate);
        template
          .find(".chat_file_size")
          .text(swipeDetails.length ? `(${swipeDetails[0]}${swipeDetails.length > 1 ? "," : ")"}` : "");
  ...
  if (swipeIdInput instanceof HTMLInputElement) {
    ...
    swipeIdInput.addEventListener("blur", () => {
      syncSwipeIdInput();
    });
  }
