...
async function onVectorizeAllClick() {
  try {
    ...
    while (!finished) {
      ...
      const processedPercent = initialPending > 0 ? Math.min(100, Math.round((processed / initialPending) * 100)) : 100;
  ...
  try {
    ...
    if (settings.summarize) {
      ...
      if (toSummarize.length > 0) {
        ...
        const failed = toSummarize.filter((x) => (x as any).summaryFailed);
        if (failed.length > 0) {
          for (const item of failed) skippedHashes.add(item.hash);
          batch = batch.filter((x) => !(x as any).summaryFailed);
        }
      }
    }
...
async function processFiles(chat: any) {
  try {
    ...
    const dataBankCollectionIds = await (ingestDataBankAttachments as any)();
...
function getVectorsRequestBody(args = {}) {
  const body = Object.assign({}, args) as any;
...
export async function init() {
  ...
  $("#vectors_workers_ai_model")
    .val((settings as any).workers_ai_model)
    .on("change", () => {
      (settings as any).workers_ai_model = String($("#vectors_workers_ai_model").val());
      Object.assign(extension_settings.vectors, settings);
      saveSettingsDebounced();
    });
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      name: "db-ingest",
      callback: async () => {
        await (ingestDataBankAttachments as any)();
        return "";
      },
  ...
  SlashCommandParser.addCommandObject(
    SlashCommand.fromProps({
      ...
      unnamedArgumentList: [new SlashCommandArgument("Query to search by.", ARGUMENT_TYPE.STRING, true, false)],
