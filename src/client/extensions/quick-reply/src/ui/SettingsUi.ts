...
export class SettingsUi {
  ...
  prepareQrEditor() {
    ...
    this.dom.querySelector("#qr--colorClear").addEventListener("click", (evt: any) => {
      const qrs = this.currentQrSet;
  ...
  async onGlobalSetListSort() {
    this.settings.config.setList = Array.from(this.globalSetList.children).map((it, idx) => {
      const set = this.settings.config.setList[Number((it as Element).getAttribute("data-order"))];
  ...
  async onChatSetListSort() {
    this.settings.chatConfig.setList = Array.from(this.chatSetList.children).map((it, idx) => {
      const set = this.settings.chatConfig.setList[Number((it as Element).getAttribute("data-order"))];
      (it as Element).setAttribute("data-order", String(idx));
      return set;
    });
    this.settings.save();
  ...
  async onQrListSort() {
    this.currentQrSet.qrList = Array.from(this.qrList.querySelectorAll(".qr--set-item")).map((it, idx) => {
      const qr = this.currentQrSet.qrList.find((qr: any) => qr.id == Number((it as Element).getAttribute("data-id")));
      (it as Element).setAttribute("data-order", String(idx));
      return qr;
    });
