...
export class AutoExecuteHandler {
  ...
  /** @type {Boolean[]}*/ preventAutoExecuteStack:any[] = [];
  ...
  getCommands(eventName:any) {
    const getFromConfig = (config:any) => {
      ...
      return config?.setList?.flatMap((link:any) => (link.set ? link.set.qrList.filter((qr:any) => qr[eventName]) : [])) ?? [];
    };
    return [
  ...
  async handleWIActivation(entries:any) {
    ...
    const automationIds = entries.map((entry:any) => entry.automationId).filter(Boolean);
    if (automationIds.length === 0) return;

    const getFromConfig = (config:any) => {
      return (
        config?.setList?.flatMap((link:any) =>
          link.set ? link.set.qrList.filter((qr:any) => qr.automationId && automationIds.includes(qr.automationId)) : [],
        ) ?? []
      );
    };
