...
async function loadSettingOptions() {
  return Promise.all([loadSamplers(), loadModels(), loadSchedulers(), loadVaes(), loadComfyWorkflows()]);
}
...
async function loadNovelSamplers() {
  return ["k_euler_ancestral", "k_euler", "k_dpmpp_2m", "k_dpmpp_sde", "k_dpmpp_2s_ancestral", "k_dpm_fast", "ddim"];
}
...
async function generateComfyImageCommon(prompt: any, negativePrompt: any, signal: any, basePath: any, placeholders: any, url: any) {
  ...
  const seed =
    extension_settings.sd.seed >= 0 ? extension_settings.sd.seed : Math.round(Math.random() * Number.MAX_SAFE_INTEGER);
  workflow = workflow.replaceAll('"%seed%"', JSON.stringify(seed));

  const denoising_strength =
    extension_settings.sd.denoising_strength === undefined ? 1.0 : extension_settings.sd.denoising_strength;
...
async function onComfyDeleteWorkflowClick() {
  const confirm = await callGenericPopup(t`Delete the workflow? This action is irreversible.`, POPUP_TYPE.CONFIRM, "", {
    okButton: t`Delete`,
    cancelButton: t`Cancel`,
  });
  if (!confirm) {
...
async function generateMediaSwipe(
  ...
  try {
    ...
    const refineArgs = {
      ...
      resolution:
        mediaAttachment.width && mediaAttachment.height ? `${mediaAttachment.width}x${mediaAttachment.height}` : null,
    };
...
export async function init() {
  ...
  if (!CSS.supports("field-sizing", "content")) {
    $(".sd_settings .inline-drawer-toggle").on("click", () => {
