...
class CoquiTtsProvider {
  ...
  async loadSettings(settings: any) {
    ...
    $("#coqui_model_origin").on("change", () => {
      this.onModelOriginChange();
    });
    $("#coqui_api_language").on("change", () => {
      this.onModelLanguageChange();
    });
    $("#coqui_api_model_name").on("change", () => {
      this.onModelNameChange();
    });

    $("#coqui_remove_voiceId_mapping").on("click", () => {
      this.onRemoveClick();
    });
    $("#coqui_add_voiceId_mapping").on("click", () => {
      this.onAddClick();
    });
  ...
  updateCustomVoices() {
    ...
    for (const voiceName in this.settings.voiceMapDict) {
      ...
      if (voiceId.model_language != null) this.settings.customVoices[voiceName] += "[" + voiceId.model_language + "]";

      if (voiceId.model_speaker != null) this.settings.customVoices[voiceName] += "[" + voiceId.model_speaker + "]";
    }
  ...
  async onAddClick() {
    ...
    const model_origin = $("#coqui_model_origin").val() as string;
    const model_language = $("#coqui_api_language").val() as string;
    const model_name = $("#coqui_api_model_name").val() as string;
    ...
    if (!voiceName) {
      toastr.error("Voice name empty, please enter one.", DEBUG_PREFIX + " voice mapping voice name", {
        timeOut: 10000,
    ...
    if (model_origin == "none") {
      toastr.error("Origin not selected, please select one.", DEBUG_PREFIX + " voice mapping origin", {
        timeOut: 10000,
        extendedTimeOut: 20000,
        preventDuplicates: true,
      });
    ...
    if (model_origin == "local") {
      ...
      if (model_name == "none") {
        toastr.error("Model not selected, please select one.", DEBUG_PREFIX + " voice mapping model", {
          timeOut: 10000,
          extendedTimeOut: 20000,
          preventDuplicates: true,
        });
    ...
    if (model_language == "none") {
      toastr.error("Language not selected, please select one.", DEBUG_PREFIX + " voice mapping language", {
        timeOut: 10000,
        extendedTimeOut: 20000,
        preventDuplicates: true,
      });
    ...
    let modelDict = coquiApiModels as any;
    if (model_origin == "coqui-api-full") modelDict = coquiApiModelsFull;

    if ((((model_setting_language == null) as any) & (("languages" in modelDict[model_language][model_dataset][model_label]) as any))) {
      toastr.error("Model language not selected, please select one.", DEBUG_PREFIX + " voice mapping model language", {
        timeOut: 10000,
        extendedTimeOut: 20000,
        preventDuplicates: true,
      });
    ...
    if ((((model_setting_speaker == null) as any) & (("speakers" in modelDict[model_language][model_dataset][model_label]) as any))) {
      toastr.error("Model speaker not selected, please select one.", DEBUG_PREFIX + " voice mapping model speaker", {
        timeOut: 10000,
        extendedTimeOut: 20000,
        preventDuplicates: true,
      });
  ...
  async onRemoveClick() {
    const voiceName = $("#coqui_voicename_select").val() as string;

    if (voiceName === "none") {
      toastr.error("Voice not selected, please select one.", DEBUG_PREFIX + " voice mapping voiceId", {
        timeOut: 10000,
        extendedTimeOut: 20000,
        preventDuplicates: true,
      });
  ...
  async onModelLanguageChange() {
    ...
    for (const model_dataset in modelDict[model_language])
      for (const model_name in modelDict[model_language][model_dataset]) {
  ...
  async onModelNameChange() {
    ...
    if ("languages" in model_settings) {
      ...
      for (let i = 0; i < model_settings.languages.length; i++) {
        ...
        $("#coqui_api_model_settings_language").append(new Option(language_label, String(i)));
      }
    } else {
    ...
    if ("speakers" in model_settings) {
      ...
      for (let i = 0; i < model_settings.speakers.length; i++) {
        ...
        $("#coqui_api_model_settings_speaker").append(new Option(speaker_label, String(i)));
      }
    } else {
    ...
    if (model_state == "installed") {
      ...
      if (model_state == "corrupted") {
        ...
        toastr.info(
          "Click download button to install the model " + $("#coqui_api_model_name").find(":selected").text(),
        ...
        .on("click", async () => {
          try {
            ...
            if (apiResultJSON.status == "downloading") {
              toastr.error("Check extras console for progress", DEBUG_PREFIX + " already downloading", {
                timeOut: 10000,
                extendedTimeOut: 20000,
                preventDuplicates: true,
              });
