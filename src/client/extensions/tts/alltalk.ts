...
class AllTalkTtsProvider {
  ...
  get settingsHtml() {
    ...
    for (const language in this.languageLabels) {
      html += `<option value='${(this.languageLabels as Record<string, any>)[language]}' ${(this.languageLabels as Record<string, any>)[language] === this.settings?.language ? 'selected="selected"' : ""}>${language}</option>`;
    }
    html += `</select>
        </div>
    </div>`;
    ...
    if (this.rvcVoices) {
      for (const rvccharvoice of this.rvcVoices) {
    ...
    if (this.rvcVoices) {
      for (const rvcnarrvoice of this.rvcVoices) {
...
<span>- <strong>Text-generation-webui</strong> users - Uncheck <strong>Enable TTS</strong> in the TGWUI interface, or you will hear 2x voices and file names being generated.</span>
  ...
  applySettingsToHTML() {
    const narratorVoiceSelect = document.getElementById("narrator_voice") as HTMLSelectElement;
    const atNarratorSelect = document.getElementById("at_narrator_enabled") as HTMLSelectElement;
    const textNotInsideSelect = document.getElementById("at_narrator_text_not_inside") as HTMLSelectElement;
    const generationMethodSelect = document.getElementById("at_generation_method") as HTMLSelectElement;
    ...
    if (atNarratorSelect) {
      const ttsPassAsterisksCheckbox = document.getElementById("tts_pass_asterisks") as HTMLInputElement;
      const ttsNarrateQuotedCheckbox = document.getElementById("tts_narrate_quoted") as HTMLInputElement;
      const ttsNarrateDialoguesCheckbox = document.getElementById("tts_narrate_dialogues") as HTMLInputElement;
    ...
    const languageSelect = document.getElementById("language_options") as HTMLSelectElement;
  ...
  updateRvcVoiceDropdowns() {
    ...
    const rvcCharacterVoiceSelect = document.getElementById("rvc_character_voice") as HTMLSelectElement;
    if (rvcCharacterVoiceSelect) {
      rvcCharacterVoiceSelect.disabled = !isV2;
      if (this.rvcVoices) {
        rvcCharacterVoiceSelect.innerHTML = "";
        for (const voice of this.rvcVoices) {
    ...
    const rvcNarratorVoiceSelect = document.getElementById("rvc_narrator_voice") as HTMLSelectElement;
    ...
    // Update pitch inputs
    const characterPitch = document.getElementById("rvc_character_pitch") as HTMLInputElement;
    if (characterPitch) {
      characterPitch.disabled = !isV2;
    }

    const narratorPitch = document.getElementById("rvc_narrator_pitch") as HTMLInputElement;
    if (narratorPitch) {
  ...
  updateCheckboxes() {
    const deepspeedCheckbox = document.getElementById("deepspeed") as HTMLInputElement;
    const lowVramCheckbox = document.getElementById("low_vram") as HTMLInputElement;
  ...
  updateLanguageDropdown() {
    ...
    if (languageSelect) {
      ...
      for (const language in this.languageLabels) {
        const option = document.createElement("option");
        option.value = (this.languageLabels as Record<string, any>)[language];
  ...
  setupEventListeners() {
    ...
    if (serverVersionSelect) {
      serverVersionSelect.addEventListener("change", async (event) => {
        this.settings.server_version = (event.target as HTMLInputElement).value;
        this.onSettingsChange();
        if ((event.target as HTMLInputElement).value === "v2") {
    ...
    if (rvcCharacterVoiceSelect) {
      rvcCharacterVoiceSelect.addEventListener("change", (event) => {
        this.settings.rvccharacter_voice_gen = (event.target as HTMLInputElement).value;
        this.onSettingsChange();
      });
    }
    ...
    if (rvcNarratorVoiceSelect) {
      rvcNarratorVoiceSelect.addEventListener("change", (event) => {
        this.settings.rvcnarrator_voice_gen = (event.target as HTMLInputElement).value;
        this.onSettingsChange();
      });
    }
    ...
    if (rvcCharacterPitchSelect) {
      rvcCharacterPitchSelect.addEventListener("change", (event) => {
        this.settings.rvc_character_pitch = (event.target as HTMLInputElement).value;
        this.onSettingsChange();
      });
    }
    ...
    if (rvcNarratorPitchSelect) {
      rvcNarratorPitchSelect.addEventListener("change", (event) => {
        this.settings.rvc_narrator_pitch = (event.target as HTMLInputElement).value;
        this.onSettingsChange();
      });
    }
    ...
    if (deepspeedCheckbox) {
      const handleDeepSpeedChange = async (event: any) => {
        const deepSpeedValue = (event.target as HTMLInputElement).checked ? "True" : "False";
        updateStatus("Processing");
        try {
          const response = await fetch(
    ...
    if (lowVramCheckbox) {
      const handleLowVramChange = async (event: any) => {
        const lowVramValue = (event.target as HTMLInputElement).checked ? "True" : "False";
        updateStatus("Processing");
        try {
          const response = await fetch(
    ...
    if (narratorVoiceSelect) {
      narratorVoiceSelect.addEventListener("change", (event) => {
        this.settings.narrator_voice_gen = `${(event.target as HTMLInputElement).value}`;
        this.onSettingsChange();
      });
    }
    ...
    if (textNotInsideSelect) {
      textNotInsideSelect.addEventListener("change", (event) => {
        this.settings.text_not_inside = (event.target as HTMLInputElement).value;
        this.onSettingsChange();
      });
    }
    ...
    if (atNarratorSelect && textNotInsideSelect && narratorVoiceSelect) {
      atNarratorSelect.addEventListener("change", (event) => {
        const narratorOption = (event.target as HTMLInputElement).value;
    ...
    if (languageSelect) {
      languageSelect.addEventListener("change", (event) => {
        this.settings.language = (event.target as HTMLInputElement).value;
        this.onSettingsChange();
      });
    }
