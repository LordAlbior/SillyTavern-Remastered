...
import { filterValidIpPatterns, getIpFromRequest, getRealOrForwardedIp } from "../express-common";
...
whitelist = filterValidIpPatterns(
  whitelist,
  (entry, message) => `${color.red("Warning")}: Ignoring invalid whitelist entry ${color.yellow(entry)} - ${message}`,
);
...
export default async function getWhitelistMiddleware(): Promise<import("express").RequestHandler> {
  ...
  return (req, res, next) => {
    ...
    if (!isIPInWhitelist(whitelist, clientIp) || (forwardedIp && !isIPInWhitelist(whitelist, forwardedIp))) {
