...
app.get("/", cacheBuster.middleware, (request, response) => {
  ...
  return response.sendFile("index.html", { root: path.join(rootDirectory, "src", "pages") });
});
...
app.use("/scripts/extensions", express.static(path.join(rootDirectory, "src", "client", "extensions"), { index: false }));
app.use("/scripts", express.static(path.join(rootDirectory, "src", "client", "dist", "scripts"), {}));
