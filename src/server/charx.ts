...
export class CharXParser {
  ...
  constructor(data: ArrayBufferLike | Uint8Array) {
    ...
    this.#data = findZipStart(
      Buffer.isBuffer(data) ? (data as Buffer) : Buffer.from(data as ArrayBufferLike),
  ...
  collectCharXAssets(card: Record<string, unknown>): CharXAsset[] {
    ...
    return assets
      .map((asset: unknown, index: number): CharXAsset | null => {
...
export function persistCharXAssets(
  ...
  directories: UserDirectoriesForCharX,
