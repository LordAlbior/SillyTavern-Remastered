...
router.post("/install", async (request, response) => {
  try {
    ...
    if (!extensionNameSanitized) {
      return response
        .status(400)
        .send("Could not determine the extension name from the URL. Please provide a valid git repository URL.");
    }
...
router.post("/branches", async (request, response) => {
  try {
    ...
    const result = [...Object.values(localBranches.branches), ...Object.values(remoteBranches.branches)].map((b) => ({
      current: b.current,
      commit: b.commit,
      name: b.name,
      label: b.label,
...
router.post("/move", async (request, response) => {
  try {
    ...
    if (!extensionNameSanitized || !source || !destination) {
      return response
        .status(400)
        .send("Bad Request: A valid extensionName, source, and destination are required in the request body.");
    }
    ...
    const sourceDirectory =
      source === "global" ? PUBLIC_DIRECTORIES.globalExtensions : request.user.directories.extensions;
    const destinationDirectory =
      destination === "global" ? PUBLIC_DIRECTORIES.globalExtensions : request.user.directories.extensions;
