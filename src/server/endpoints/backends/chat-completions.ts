...
async function sendClaudeRequest(request: import('express').Request, response: import('express').Response) {
  ...
  try {
    ...
    const useThinking = /^claude-(3-7|opus-4|sonnet-4|haiku-4-5|opus-4-5|opus-4-6|sonnet-4-6|opus-4-7)/.test(
      request.body.model,
    );
    const useWebSearch =
      /^claude-(3-5|3-7|opus-4|sonnet-4|haiku-4-5|opus-4-5|opus-4-6|sonnet-4-6|opus-4-7)/.test(request.body.model) &&
      Boolean(request.body.enable_web_search);
    const isLimitedSampling = /^claude-(opus-4-1|sonnet-4-5|haiku-4-5|opus-4-5|opus-4-6|sonnet-4-6)/.test(
      request.body.model,
    );
...
async function sendMakerSuiteRequest(request: import('express').Request, response: import('express').Response) {
  ...
  const responseSchema =
    request.body.responseSchema ?? (request.body.json_schema ? request.body.json_schema.value : undefined);
...
async function sendDeepSeekRequest(request: import('express').Request, response: import('express').Response) {
  ...
  try {
    ...
    const processedMessages = addAssistantPrefix(
      postProcessPrompt(request.body.messages, PROMPT_PROCESSING_TYPE.SEMI_TOOLS, getPromptNames(request)),
      bodyParams.tools,
      "prefix",
    );
...
router.post("/status", async (request, statusResponse) => {
  try {
    ...
    if (request.body.chat_completion_source === CHAT_COMPLETION_SOURCES.OPENAI) {
      ...
      const defaultApiUrl =
        request.body.siliconflow_endpoint === SILICONFLOW_ENDPOINT.CN ? API_SILICONFLOW_CN : API_SILICONFLOW;
...
router.post("/bias", async (request, response) => {
...
router.post("/generate", async (request, response) => {
  try {
    ...
    if (
      request.body.reasoning_effort &&
      [CHAT_COMPLETION_SOURCES.CUSTOM, CHAT_COMPLETION_SOURCES.OPENAI].includes(request.body.chat_completion_source)
    ) {
