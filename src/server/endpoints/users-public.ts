...
import { KEY_PREFIX, getUserAvatar, toKey, getPasswordHash, getPasswordSalt, getAccountVersion } from "../users";
...
router.post("/list", async (_request, response) => {
  try {
    ...
    const users = (await storage.values((x) => x.key.startsWith(KEY_PREFIX))) as any[];

    const viewModelPromises: Promise<any>[] = users
      .filter((x) => x.enabled)
      .map(
        (user) =>
