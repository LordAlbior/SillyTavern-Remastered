...
async function extractTranscript(videoPageBody: string, lang: string | null): Promise<string> {
  ...
  const transcriptURL = (
    lang ? captions.captionTracks.find((track: CaptionTrack) => track.languageCode === lang) : captions.captionTracks[0]
  ).baseUrl;
...
router.post("/transcript", async (request, response) => {
  try {
    ...
    try {
      ...
      return json ? response.json({ transcript: transcriptText, html: videoPageBody }) : response.send(transcriptText);
    } catch (error) {
      if (json) {
