  ...
  return [...backgrounds].sort((a, b) => {
    switch (sortOrder) {
      ...
      case BG_SORT_OPTIONS.OLDEST: {
        ...
        return sortOrder === BG_SORT_OPTIONS.NEWEST ? timestampB - timestampA : timestampA - timestampB;
      }
      default:
        return 0;
    }
...
async function convertFileIfVideo(formData) {
  ...
  try {
    toastMessage = toastr.info(t`Preparing video for upload. This may take several minutes.`, t`Please wait`, {
      timeOut: 0,
      extendedTimeOut: 0,
    });
