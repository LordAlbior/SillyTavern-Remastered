...
const interleaved_reasoning_providers = [chat_completion_sources.OPENROUTER, chat_completion_sources.CUSTOM];
...
async function validateReverseProxy() {
  ...
  const confirmation =
    ...
    (await Popup.show.confirm(
      ...
      await renderTemplateAsync("proxyConnectionWarning", { proxyURL: DOMPurify.sanitize(oai_settings.reverse_proxy) }),
    ));
    ...
    switch (oai_settings.names_behavior) {
      ...
      case character_names_behavior.DEFAULT:
        if (
          ...
          (chat[j].force_avatar && chat[j].name !== name1 && chat[j].extra?.type !== system_message_types.NARRATOR)
        ) {
...
export async function createGenerationParameters(settings: any, model: any, type: any, messages: any, { jsonSchema = null }: any = {}) {
  ...
  if (
    [chat_completion_sources.MAKERSUITE, chat_completion_sources.VERTEXAI].includes(settings.chat_completion_source)
  ) {
...
function loadOpenAISettings(data: any, settings: any) {
  ...
  $(`#settings_preset_openai option[value="${openai_setting_names[oai_settings.preset_settings_openai]}"]`).prop(
    "selected",
    true,
  );
...
async function getStatusOpen() {
  ...
  const canBypass =
    (oai_settings.chat_completion_source === chat_completion_sources.OPENAI && oai_settings.bypass_status_check) ||
...
async function onModelChange(this: any) {
  ...
  if (
    [chat_completion_sources.MAKERSUITE, chat_completion_sources.VERTEXAI].includes(oai_settings.chat_completion_source)
  ) {
  ...
  if (
    [chat_completion_sources.AZURE_OPENAI, chat_completion_sources.OPENAI].includes(oai_settings.chat_completion_source)
  ) {
  ...
  if (oai_settings.chat_completion_source === chat_completion_sources.COHERE) {
    if (oai_settings.max_context_unlocked) {
      ...
      ["c4ai-aya-23", "c4ai-aya-expanse-32b", "command-nightly", "command-a-vision-07-2025"].includes(
  ...
  if (oai_settings.chat_completion_source === chat_completion_sources.AIMLAPI) {
    ...
    $("#temp_openai").prop("max", oai_max_temp).val(Number(oai_settings.temp_openai)).trigger("input");
...
export function isImageInliningSupported() {
  ...
  switch (oai_settings.chat_completion_source) {
    ...
    case chat_completion_sources.MISTRALAI:
      return (
        Array.isArray(model_list) && model_list.find((m) => m.id === oai_settings.mistralai_model)?.capabilities?.vision
      );
    case chat_completion_sources.COHERE:
    ...
    case chat_completion_sources.AIMLAPI:
      return (
        Array.isArray(model_list) &&
        model_list.find((m) => m.id === oai_settings.aimlapi_model)?.features?.includes("openai/chat-completion.vision")
      );
    case chat_completion_sources.CHUTES:
      return (
        Array.isArray(model_list) &&
        model_list.find((m) => m.id === oai_settings.chutes_model)?.input_modalities?.includes("image")
      );
    case chat_completion_sources.ELECTRONHUB:
      return (
        Array.isArray(model_list) && model_list.find((m) => m.id === oai_settings.electronhub_model)?.metadata?.vision
      );
    case chat_completion_sources.POLLINATIONS:
      return (
        Array.isArray(model_list) &&
        model_list.find((m) => m.id === oai_settings.pollinations_model)?.input_modalities?.includes("image")
      );
    case chat_completion_sources.COMETAPI:
      return true;
    case chat_completion_sources.MOONSHOT:
      return (
        Array.isArray(model_list) && model_list.find((m) => m.id === oai_settings.moonshot_model)?.supports_image_in
      );
    case chat_completion_sources.NANOGPT:
      return (
        Array.isArray(model_list) && model_list.find((m) => m.id === oai_settings.nanogpt_model)?.capabilities?.vision
      );
    case chat_completion_sources.ZAI:
...
export function initOpenAI() {
  ...
  $("#newexamplechat_prompt_restore").on("click", () => {
  ...
  $("#continue_nudge_prompt_restore").on("click", () => {
  ...
  $("#group_nudge_prompt_restore").on("click", () => {
  ...
  $("#continue_postfix_none").on("input", () => {
    oai_settings.continue_postfix = continue_postfix_types.NONE;
    setContinuePostfixControls();
    saveSettingsDebounced();
  });

  $("#continue_postfix_space").on("input", () => {
    oai_settings.continue_postfix = continue_postfix_types.SPACE;
    setContinuePostfixControls();
    saveSettingsDebounced();
  });

  $("#continue_postfix_newline").on("input", () => {
    oai_settings.continue_postfix = continue_postfix_types.NEWLINE;
    setContinuePostfixControls();
    saveSettingsDebounced();
  });

  $("#continue_postfix_double_newline").on("input", () => {
