// (no script imports — i18n is a leaf module; dependencies inject hooks into it)
...
async function getMissingTranslations() {
  ...
  if (trackMissingDynamicTranslate) {
    missingData.push(
      ...Array.from(trackMissingDynamicTranslate).map((key) => ({ key, language: localeFile, value: key })),
    );
  }
