...
export class MacroClosingTagAutoCompleteOption extends AutoCompleteOption {
  ...
  renderItem() {
    ...
    if (this.#isOptional) {
      ...
      const nestingInfo =
        this.#nestingLevel > 0 ? ` (nested ${this.#nestingLevel} level${this.#nestingLevel > 1 ? "s" : ""} deep)` : "";
...
export function parseMacroContext(macroText: any, cursorOffset: any) {
  ...
  const identifierEndPos =
    identifierStartPos + (firstPartText.length - firstPartText.trimStart().length) + identifierOnly.length;
