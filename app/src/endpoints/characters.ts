...
async function importFromByaf(uploadPath: string, { request }: { request: any }, preservedFileName: string | undefined) {
  ...
  const fileName =
    preservedFileName ||
    getPngName(
      sanitize(byafData.character.displayName || card.name, { replacement: sanitizeSafeCharacterReplacements }),
...
router.post("/create", getFileNameValidationFunction("file_name"), async (request, response) => {
  try {
...
router.post("/edit", validateAvatarUrlMiddleware, async (request, response) => {
  if (!request.body) {
...
router.post("/edit-avatar", validateAvatarUrlMiddleware, async (request, response) => {
  try {
    if (!request.file) {
 ...
 * @returns {void}
 */
router.post("/edit-attribute", validateAvatarUrlMiddleware, async (request, response) => {
 ...
 * @returns {void}
 */
router.post("/merge-attributes", getFileNameValidationFunction("avatar"), async (request, response) => {
  try {
...
router.post("/delete", validateAvatarUrlMiddleware, async (request, response) => {
  if (!request.body || !request.body.avatar_url) {
    return response.sendStatus(400);
  }
...
router.post("/get", validateAvatarUrlMiddleware, async (request, response) => {
  try {
    if (!request.body) return response.sendStatus(400);
...
router.post("/chats", validateAvatarUrlMiddleware, async (request, response) => {
  try {
    if (!request.body) return response.sendStatus(400);
...
router.post("/duplicate", validateAvatarUrlMiddleware, async (request, response) => {
  try {
...
router.post("/export", validateAvatarUrlMiddleware, async (request, response) => {
  try {
