...
router.post("/generate-voice", async (request, response) => {
  try {
    ...
    if (!text || !voiceId || !apiKey || !groupId) {
      ...
      return response
        .status(400)
        .json({ error: "Missing required parameters: text, voiceId, apiKey, and groupId are required" });
    }
    ...
    if (!apiResponse.ok) {
      ...
      try {
        ...
        if (baseResp && baseResp.status_code !== 0) {
          ...
          errorMessage =
            errorData.error?.message || errorData.message || errorData.detail || `HTTP ${apiResponse.status}`;
        }
      } catch (jsonError) {
