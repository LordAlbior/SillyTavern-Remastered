...
async function getBatchVector(source: string, sourceSettings: any, texts: string[], isQuery: boolean, directories: any) {
  ...
  for (const batch of batches) {
    switch (source) {
      ...
      case "siliconflow":
        results.push(
          ...(await getOpenAIBatchVector(batch, source, directories, sourceSettings.model, sourceSettings.urlOverride)),
        );
        break;
      case "workers_ai":
        results.push(
  ...
  switch (source) {
    ...
    case "siliconflow":
      return {
        ...
        urlOverride: request.body.siliconflow_endpoint === "cn" ? "https://api.siliconflow.cn/v1" : null,
      };
    case "workers_ai": {
