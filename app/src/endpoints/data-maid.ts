...
export class DataMaidService {
    ...
    try {
      ...
      for (const folder of chatFolders) {
        if (folder.isDirectory() && !knownChatFolders.has(folder.name)) {
          const chatFiles = await fs.promises.readdir(path.join(this.directories.chats, folder.name), {
            withFileTypes: true,
          });
    ...
    try {
      ...
      for (const directory of chatDirectories) {
        if (directory.isDirectory()) {
          const chatFiles = await fs.promises.readdir(path.join(this.directories.chats, directory.name), {
            withFileTypes: true,
          });
          for (const file of chatFiles) {
            if (file.isFile() && path.parse(file.name).ext === ".jsonl") {
  ...
  static generateToken(handle, report) {
    ...
    DataMaidService.TOKENS.set(token, tokenEntry);
    return token;
  }
}
