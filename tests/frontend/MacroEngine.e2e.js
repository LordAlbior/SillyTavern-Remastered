...
test.describe("MacroEngine", () => {
  ...
  test.describe("Type validation", () => {
    test("should not resolve strict typed macro when argument type is invalid", async ({ page }) => {
      ...
      expect(
        warnings.some((w) => w.includes('Macro "test-int-strict"') && w.includes("expected type integer")),
      ).toBeTruthy();
    });
    ...
    test("should resolve non-strict typed macro when argument type is invalid but still log warning", async ({
      ...
      expect(
        warnings.some((w) => w.includes('Macro "test-int-nonstrict"') && w.includes("expected type integer")),
      ).toBeTruthy();
    });
  });
      ...
      test("should support optional arguments in dynamic macro", async ({ page }) => {
        const output = await page.evaluate(async () => {
          ...
          const rawEnv = {
            ...
            dynamicMacros: {
              greet: {
                unnamedArgs: [{ name: "name" }, { name: "greeting", optional: true, defaultValue: "Hello" }],
  ...
  test.describe("Variable Shorthand Syntax", () => {
    ...
    test("should support nested macro in variable value", async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, "{{.greeting = Hello {{user}}}}{{.greeting}}", {
        local: {},
      });
  ...
  test.describe("Variable Shorthand in {{if}} Macro", () => {
    ...
    test("should treat non-existent variable as falsy in if condition", async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, "{{if .nonexistent}}Yes{{else}}No{{/if}}", {
        local: {},
      });
      expect(output).toBe("No");
    });
    ...
    test("should handle else branch with variable shorthand", async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, "{{if .active}}On{{else}}Off{{/if}}", {
        local: { active: "yes" },
      });
    ...
    test("should work with variable set before if check", async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, "{{.ready = yes}}{{if .ready}}Ready!{{/if}}", {
        local: {},
      });
    ...
    test("should treat zero as falsy in if condition", async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, "{{if .count}}Has count{{else}}No count{{/if}}", {
        local: { count: "0" },
      });
    ...
    test("should treat non-zero number as truthy in if condition", async ({ page }) => {
      const output = await evaluateWithEngineAndVariables(page, "{{if .count}}Count: {{.count}}{{/if}}", {
        local: { count: "42" },
      });
