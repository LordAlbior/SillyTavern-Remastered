  ...
  test.describe("{{var}} Macro", () => {
    ...
    test("should support {{var::key::index}} macro", async ({ page }) => {
      const output = await page.evaluate(async () => {
        ...
        return (
          await executeSlashCommandsWithOptions('/let key=list ["item1","item2","item3"] || /pass {{var::list::1}}')
        ).pipe;
      });
    ...
    test("should not fail on out-of-bounds variable index", async ({ page }) => {
      const output = await page.evaluate(async () => {
        ...
        return (await executeSlashCommandsWithOptions('/let key=test {"key":"value"} || /pass {{var::test::error}}'))
          .pipe;
      });
