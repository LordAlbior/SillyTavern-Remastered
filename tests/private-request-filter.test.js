...
describe("private request filter", () => {
  test("allows direct private IP requests only when whitelisted", async () => {
    ...
    await expect(blockedAgent.connect({}, { host: "127.0.0.1", secureEndpoint: false })).rejects.toThrow(
  ...
  test("handles unresolved hosts according to allowUnresolvedHosts setting", async () => {
    ...
    await expect(blockedAgent.connect({}, { host: "missing-host.local", secureEndpoint: false })).rejects.toThrow(
