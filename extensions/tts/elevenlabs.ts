...
class ElevenLabsTtsProvider {
  ...
  constructor() {
    this.handler = async function (this: any, /** @type {string} */ key: any) {
      ...
      $("#elevenlabs_tts_key").toggleClass("success", !!(secret_state as Record<string, any>)[SECRET_KEYS.ELEVENLABS]);
  ...
  async loadSettings(settings: any) {
    ...
    if (Object.hasOwn(settings, "apiKey")) {
      if (settings.apiKey && !(secret_state as Record<string, any>)[SECRET_KEYS.ELEVENLABS]) {
        await (writeSecret as any)(SECRET_KEYS.ELEVENLABS, settings.apiKey);
      }
  ...
  setupVoiceCloningMenu() {
        const audioFilesInput = document.getElementById("elevenlabs_tts_audio_files") as HTMLInputElement;
    const selectedFilesListElement = document.getElementById("elevenlabs_tts_selected_files_list") as HTMLElement;
    const cloneVoiceButton = document.getElementById("elevenlabs_tts_clone_voice_button") as HTMLElement;
    const uploadAudioFileButton = document.getElementById("upload_audio_file") as HTMLElement;
    const voiceCloningNameInput = document.getElementById("elevenlabs_tts_voice_cloning_name") as HTMLInputElement;
    const voiceCloningDescriptionInput = document.getElementById(
      "elevenlabs_tts_voice_cloning_description",
    ) as HTMLInputElement;
    const voiceCloningLabelsInput = document.getElementById(
      "elevenlabs_tts_voice_cloning_labels",
    ) as HTMLInputElement;
  ...
  async getVoice(voiceName: any) {
    ...
    const match = this.voices.filter((elevenVoice: any) => elevenVoice.name == voiceName)[0];
    if (!match) {
  ...
  async fetchTtsGeneration(text: any, voiceId: any) {
    const model = this.settings.model ?? "eleven_monolingual_v1";
