...
class ChatterboxTtsProvider {
  ...
  setupEventListeners() {
    ...
    $("#chatterbox-language").on("change", (e) => {
      this.settings.language = (e.target as HTMLInputElement).value;
      this.onSettingsChange();
    });

    // Parameter sliders
    $("#chatterbox-temperature").on("input", (e) => {
      this.settings.temperature = parseFloat((e.target as HTMLInputElement).value);
      $("#chatterbox-temperature-value").text(this.settings.temperature);
      this.onSettingsChange();
    });

    $("#chatterbox-exaggeration").on("input", (e) => {
      this.settings.exaggeration = parseFloat((e.target as HTMLInputElement).value);
      $("#chatterbox-exaggeration-value").text(this.settings.exaggeration);
      this.onSettingsChange();
    });

    $("#chatterbox-cfg-weight").on("input", (e) => {
      this.settings.cfg_weight = parseFloat((e.target as HTMLInputElement).value);
      $("#chatterbox-cfg-weight-value").text(this.settings.cfg_weight);
      this.onSettingsChange();
    });

    $("#chatterbox-speed").on("input", (e) => {
      this.settings.speed_factor = parseFloat((e.target as HTMLInputElement).value);
      $("#chatterbox-speed-value").text(this.settings.speed_factor);
      this.onSettingsChange();
    });
    ...
    $("#chatterbox-seed").on("change", (e) => {
      this.settings.seed = parseInt((e.target as HTMLInputElement).value);
      this.onSettingsChange();
    });

    // Text splitting
    $("#chatterbox-split-text").on("change", (e) => {
      this.settings.split_text = (e.target as HTMLInputElement).checked;
    ...
    $("#chatterbox-chunk-size").on("change", (e) => {
      this.settings.chunk_size = parseInt((e.target as HTMLInputElement).value);
      this.onSettingsChange();
    });

    // Output format
    $("#chatterbox-format").on("change", (e) => {
      this.settings.output_format = (e.target as HTMLInputElement).value;
      this.onSettingsChange();
    });
  }
