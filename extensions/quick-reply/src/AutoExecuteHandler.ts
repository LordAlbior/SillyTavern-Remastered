...
export class AutoExecuteHandler {
  ...
  getCommands(eventName) {
    const getFromConfig = (config) => {
      ...
      return config?.setList?.flatMap((link) => (link.set ? link.set.qrList.filter((qr) => qr[eventName]) : [])) ?? [];
    };
    return [
  ...
  async handleWIActivation(entries) {
    ...
    const getFromConfig = (config) => {
      return (
        config?.setList?.flatMap((link) =>
          link.set ? link.set.qrList.filter((qr) => qr.automationId && automationIds.includes(qr.automationId)) : [],
        ) ?? []
      );
    };
